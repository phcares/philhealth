...
$(document).on('click', '.resetPassBtn', function (event) {
	...
	auth.sendPasswordResetEmail(emailAddress).then(function() {
	  // Email sent.
		Materialize.toast('An Email has been sent to '+ emailAddress +' on how to reset his/her password.', 5000);
...
function isReportLate(){
	...
	if(value > x || value < z){
			$('.reportTimeInIndicator').removeClass('red');
			$('.reportTimeInIndicator').removeClass('white-text');
		if(value > late){
			console.log('late');
			$('.reportTimeInIndicator').addClass('red');
			$('.reportTimeInIndicator').addClass('white-text');
		}
	}
